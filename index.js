...
async function cloneRepo(repoName) {
  ...
  try {
    const branch = 'master';
    const files = await dgit.listFiles(repoName, branch);
    if (!files?.length) return console.error(`❌ No files found for repo '${repoName}'.`);

    fs.mkdirSync(DGIT_DIR);
    writeConfig({ repoName, currentBranch: branch, branches: [branch] });
    writeCommits({});
    writeStaged([]);
...
async function pullChanges() {
  ...
  console.log(`⬇️ Pulling latest files from canister for '${branch}'...`);
  try {
    const files = await dgit.listFiles(config.repoName, branch);
    if (!files?.length) return console.log('ℹ️ No files found on remote.');

    for (const f of files) {
      const content = await dgit.getFileContent(config.repoName, branch, f);
